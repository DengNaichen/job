...
class TestJobLocationRepository:
    ...
    async def test_link_idempotency(self, session: AsyncSession, setup_models):
        ...
        # First link
        link1 = await repo.link(
            job_id=job.id, location_id=loc1.id, is_primary=True, source_raw="test"
        )
        ...
        await repo.link(
            job_id=job.id, location_id=loc1.id, is_primary=False, source_raw="test updated"
        )
    ...
    async def test_set_primary(self, session: AsyncSession, setup_models):
        ...
        links = await repo.list_by_job_id(job.id)
        loc1_link = next(link for link in links if link.location_id == loc1.id)
        loc2_link = next(link for link in links if link.location_id == loc2.id)
        ...
        # Check new state
        links_after = await repo.list_by_job_id(job.id)
        loc1_link_after = next(link for link in links_after if link.location_id == loc1.id)
        loc2_link_after = next(link for link in links_after if link.location_id == loc2.id)
