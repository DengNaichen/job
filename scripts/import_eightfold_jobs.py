#!/usr/bin/env python3
"""Import Eightfold jobs with same-source full snapshot reconcile.

Create sources manually via ``POST /api/v1/sources`` before running this script.

Microsoft:
    {"name": "Microsoft", "platform": "eightfold", "identifier": "microsoft"}

NVIDIA:
    {"name": "NVIDIA", "platform": "eightfold", "identifier": "nvidia"}

Examples:
    uv run python scripts/import_eightfold_jobs.py --slug microsoft
    uv run python scripts/import_eightfold_jobs.py --slug nvidia
    uv run python scripts/import_eightfold_jobs.py --dry-run
"""
...
async def _load_eightfold_sources(
    *,
    slug: str | None,
    limit: int | None,
) -> list[Source]:
    async with AsyncSession(engine) as session:
        statement = (
            select(Source)
            .where(
                Source.platform == PlatformType.EIGHTFOLD,
...
async def run(args: argparse.Namespace) -> None:
    ...
    sources = await _load_eightfold_sources(slug=args.slug, limit=args.limit)
    fetcher = EightfoldFetcher()
    mapper = EightfoldMapper()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Import Eightfold jobs for one or all enabled source slugs.")
    parser.add_argument(
        "--slug",
        default=None,
        help="Exact Source.identifier to import. Defaults to all enabled Eightfold sources.",
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="Fetch per-job detail payloads to populate descriptions and normalized locations.",
    )
