#!/usr/bin/env python3
"""Import Ashby jobs with same-source full snapshot reconcile."""
    ...
    *,
    ...
    async with AsyncSession(engine) as session:
        statement = (
            select(Source)
            .where(
                Source.platform == PlatformType.ASHBY,
    ...
    *,
    ...
    async with AsyncSession(engine) as session:
        ...
        return await service.sync_source(
            ...
            fetcher=fetcher,
            mapper=mapper,
            include_content=include_compensation,
            dry_run=dry_run,
        )
...
async def run(args: argparse.Namespace) -> None:
    ...
    sources = await _load_ashby_sources(slug=args.slug, limit=args.limit)
    fetcher = AshbyFetcher()
    mapper = AshbyMapper()

    print(f"target_sources={len(sources)}")
    print(f"include_compensation={args.include_compensation}")
    ...
    for idx, source in enumerate(sources, start=1):
        result = await _sync_source(
            source=source,
            include_compensation=args.include_compensation,
            fetcher=fetcher,
            mapper=mapper,
