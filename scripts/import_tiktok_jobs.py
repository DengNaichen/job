#!/usr/bin/env python3
"""Import TikTok jobs with same-source full snapshot reconcile.

Create the source manually via ``POST /api/v1/sources`` before running this script.

TikTok:
    {"name": "TikTok", "platform": "tiktok", "identifier": "tiktok"}
"""
...
async def _load_tiktok_sources(
    *,
    slug: str | None,
    limit: int | None,
) -> list[Source]:
    async with AsyncSession(engine) as session:
        statement = (
            select(Source)
            .where(
                Source.platform == PlatformType.TIKTOK,
...
async def run(args: argparse.Namespace) -> None:
    ...
    sources = await _load_tiktok_sources(slug=args.slug, limit=args.limit)
    fetcher = TikTokFetcher()
    mapper = TikTokMapper()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Import TikTok jobs for one or all enabled source slugs.")
    parser.add_argument(
        "--slug",
        default=None,
        help="Exact Source.identifier to import. Defaults to all enabled TikTok sources.",
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="Accepted for ingest parity; TikTok search payloads already contain descriptions.",
    )
