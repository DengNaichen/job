#!/usr/bin/env python3
"""Import Uber jobs with same-source full snapshot reconcile.
    ...
    *,
    ...
    async with AsyncSession(engine) as session:
        statement = (
            select(Source)
            .where(
                Source.platform == PlatformType.UBER,
...
async def run(args: argparse.Namespace) -> None:
    ...
    sources = await _load_uber_sources(slug=args.slug, limit=args.limit)
    fetcher = UberFetcher()
    mapper = UberMapper()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Import Uber jobs for one or all enabled source slugs.")
    parser.add_argument(
        "--slug",
        default=None,
        help="Exact Source.identifier to import. Defaults to all enabled Uber sources.",
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="Accepted for ingest parity; Uber search payloads already contain descriptions.",
    )
