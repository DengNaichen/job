#!/usr/bin/env python3
"""Import Greenhouse jobs with same-source full snapshot reconcile."""
...
async def _load_greenhouse_sources(
    *,
    slug: str | None,
    limit: int | None,
) -> list[Source]:
    async with AsyncSession(engine) as session:
        statement = (
            select(Source)
            .where(
                Source.platform == PlatformType.GREENHOUSE,
...
async def run(args: argparse.Namespace) -> None:
    ...
    sources = await _load_greenhouse_sources(slug=args.slug, limit=args.limit)
    fetcher = GreenhouseFetcher()
    mapper = GreenhouseMapper()
