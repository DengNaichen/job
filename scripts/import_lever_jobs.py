#!/usr/bin/env python3
"""Import Lever jobs with same-source full snapshot reconcile."""
    ...
    *,
    ...
    async with AsyncSession(engine) as session:
        statement = (
            select(Source)
            .where(
                Source.platform == PlatformType.LEVER,
...
async def run(args: argparse.Namespace) -> None:
    ...
    sources = await _load_lever_sources(slug=args.slug, limit=args.limit)
    fetcher = LeverFetcher()
    mapper = LeverMapper()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Import Lever jobs for one or all enabled source slugs.")
    parser.add_argument(
        "--slug",
        default=None,
        help="Exact Source.identifier to import. Defaults to all enabled Lever sources.",
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="Accepted for ingest parity; Lever's public postings API already returns content fields.",
    )
