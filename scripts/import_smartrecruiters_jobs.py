#!/usr/bin/env python3
"""Import SmartRecruiters jobs with same-source full snapshot reconcile."""
...
async def _load_smartrecruiters_sources(
    *,
    slug: str | None,
    limit: int | None,
) -> list[Source]:
    async with AsyncSession(engine) as session:
        statement = (
            select(Source)
            .where(
                Source.platform == PlatformType.SMARTRECRUITERS,
...
async def run(args: argparse.Namespace) -> None:
    ...
    sources = await _load_smartrecruiters_sources(slug=args.slug, limit=args.limit)
    fetcher = SmartRecruitersFetcher()
    mapper = SmartRecruitersMapper()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Import SmartRecruiters jobs for one or all enabled source slugs.")
    parser.add_argument(
        "--slug",
        default=None,
        help="Exact Source.identifier to import. Defaults to all enabled SmartRecruiters sources.",
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="Accepted for ingest parity; SmartRecruiters details are always fetched to obtain apply URLs.",
    )
