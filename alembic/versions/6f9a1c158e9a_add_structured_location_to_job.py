...
def upgrade() -> None:
    ...
    op.add_column(
        "job",
        sa.Column(
            "location_workplace_type", workplace_type, server_default="unknown", nullable=False
        ),
    )
    op.add_column("job", sa.Column("location_remote_scope", sa.String(), nullable=True))

    op.create_index(
        op.f("ix_job_location_country_code"), "job", ["location_country_code"], unique=False
    )
    op.create_index(
        op.f("ix_job_location_workplace_type"), "job", ["location_workplace_type"], unique=False
    )
