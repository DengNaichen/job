...
def upgrade() -> None:
    ...
    inspector = sa.inspect(bind)
    location_indexes = {idx["name"] for idx in inspector.get_indexes("locations")}
    if "ix_locations_canonical_key" not in location_indexes:
        op.create_index("ix_locations_canonical_key", "locations", ["canonical_key"], unique=True)

    if not inspector.has_table("job_locations"):
        op.create_table(
            "job_locations",
            sa.Column("id", sa.String(length=36), nullable=False),
    ...
    op.execute(
        sa.text(
            "CREATE UNIQUE INDEX IF NOT EXISTS ix_job_locations_primary_one_per_job "
            "ON job_locations (job_id) WHERE is_primary = true"
        )
    )
...
def downgrade() -> None:
    ...
    op.execute(sa.text("DROP INDEX IF EXISTS ix_job_locations_primary_one_per_job"))
    if inspector.has_table("job_locations"):
